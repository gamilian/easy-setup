...
#
# Usage:
#   ./terminal-setup.sh              # interactive shell choice
#   ./terminal-setup.sh --fish       # use Fish
#   ./terminal-setup.sh --zsh        # use Zsh (with fish-like plugins)
#   ./terminal-setup.sh --dry-run    # preview what would be done (no changes)
#
...
if [[ ! -d "$CONFIGS_DIR" ]]; then
    ...
    git clone --depth 1 https://github.com/gamilian/easy-setup.git "$TMPDIR_CLONE/easy-setup"
    SCRIPT_DIR="$TMPDIR_CLONE/easy-setup/terminal-setup"
    CONFIGS_DIR="$SCRIPT_DIR/configs"
fi
...
case "$OS" in
    macos)
        ...
        ;;
    debian)
        # Ghostty on Linux: check if already installed, otherwise point to the official build or skip
...
install_cli_tools_linux() {
    ...
    # zoxide — apt, with bundled installer as fallback for older Debian/Ubuntu
...
# Pick the per-OS config directory (debian + wsl share the Linux variants).
case "$OS" in
    macos)      OS_CONFIG_DIR="$CONFIGS_DIR/macos" ;;
    debian|wsl) OS_CONFIG_DIR="$CONFIGS_DIR/linux" ;;
esac
