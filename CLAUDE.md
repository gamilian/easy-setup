...
This repository is `easy-setup`, a personal setup-scripts collection. Its current module is `terminal-setup`: running `./setup.sh terminal` dispatches to `terminal-setup/terminal-setup.sh`, which installs and configures a complete terminal stack (Ghostty + Fish/Zsh + Starship + Nerd Font + a set of modern CLI tools) on **macOS**, **Debian/Ubuntu**, and **Windows WSL**. macOS is the primary, well-tested target; Linux/WSL are functional but experimental.

There is no build system, dependency manager, or compilation step. The root "source" is `setup.sh` plus module directories; the terminal module source is `terminal-setup/terminal-setup.sh` plus the static config files it deploys.

## Commands

```bash
./setup.sh                   # interactive: prompts for a setup module
./setup.sh terminal          # run terminal-setup interactively
./setup.sh terminal --fish   # non-interactive Fish
./setup.sh terminal --zsh    # non-interactive Zsh
./setup.sh terminal --dry-run # print every mutating command without executing
./terminal-setup/terminal-setup.sh --zsh  # run the terminal module directly
```
...
```bash
bash -n setup.sh
bash -n terminal-setup/terminal-setup.sh
shellcheck setup.sh terminal-setup/terminal-setup.sh  # if installed — no config file, so default rules
./setup.sh terminal --dry-run --zsh   # exercise the full logic path safely; every change is gated behind run_cmd
```
...
Root `setup.sh` is a small module selector. `terminal-setup/terminal-setup.sh` (~1000 lines) is a monolithic, top-to-bottom procedural script — there are no sourced modules. Understanding the terminal module means understanding four cross-cutting mechanisms, all defined near the top:
...
Each shell config is fully self-contained in its per-OS file — `terminal-setup/terminal-setup.sh` only copies, it never edits or appends. Aliases, fzf, zoxide, fnm, history, `set-ssh-key`, and (for Fish) the abbreviations all live directly in `terminal-setup/configs/<os>/.zshrc` / `terminal-setup/configs/<os>/config.fish`. To change Fish aliases, edit the `abbr` block in the `config.fish` files — not the scripts.

### Bundled assets (offline/fallback installs on Linux)

- `terminal-setup/bin/linux-x86_64/` ships prebuilt static binaries (`delta`, `eza`, `lazygit`, `starship`, `tldr`). On Linux the install order is apt → bundled binary (`install_bundled_bin` copies to `/usr/local/bin`). This makes installs work on machines where apt lacks newer tools. (`zoxide` similarly falls back to `terminal-setup/scripts/install-zoxide.sh`; `btop` is apt-only and is skipped with a warning if unavailable. There is no snap fallback.)
- `terminal-setup/fonts/` ships the MesloLGS NF `.ttf` files; they are copied directly (no network download).
- `terminal-setup/scripts/install-zoxide.sh` is a vendored fallback installer used when apt doesn't have zoxide.
- On Debian, `bat`→`batcat` and `fd`→`fdfind`; the script symlinks the real names into `~/.local/bin`.

### curl-pipe bootstrap

If `configs/` isn't found next to `terminal-setup/terminal-setup.sh` (i.e. it was run via `bash <(curl ...)`), Step 0 `git clone`s the repo to a tmpdir and repoints `$SCRIPT_DIR`/`$CONFIGS_DIR` to `easy-setup/terminal-setup` before proceeding.
...
- `terminal-setup/terminal-color-demo.sh` — standalone interactive True-Color/Nerd-Font demo; not part of the installer.
- `terminal-setup/dev-layout-sample.kdl` — sample Zellij layout users can load with `zellij --layout`; not deployed by the script.
